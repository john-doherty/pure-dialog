...
All pure-dialog events bubble up so it is possible to listen for a pure-dialog event on the element itself or at the root of the DOM. Assuming `var dialog = document.getElementById('example')`:
